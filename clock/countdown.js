...
var MARGIN_TOP = 60;
var MARGIN_LEFT = 30;
const endTime = new Date(2017, 3, 13, 19, 15, 18);
...
/**
 * [addBalls description]
 * @param x x轴
 * @param y y轴
 * @param num 对num点阵化的位置进行彩色小球绘制
 */
		...
		for (var j = 0; j < digit[num][i].length; j++)
			if (digit[num][i][j] == 1){
				var aBall = {
					...
					g : 1.5 + Math.random(), //1.5-2.5随机数
					vx: Math.pow(-1, Math.ceil(Math.random() * 1000)) * 4, //取-4或者4  
					vy: -5,                                                //Math.pow(-1, Math.ceil(Math.random() * 1000)) 取-1或者+1                       
